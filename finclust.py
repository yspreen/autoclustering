...
def accuracy(res):
  acc = pd.DataFrame(columns=['m_id', 'r_id', 'max', 'acc_correct', 'acc_total'])
  gres = res.groupby('Group')
  #group_labels = {}
  for i in gres.groups:
    mx = {'max': -1}
    for j in gmeta.groups:
      m = pd.merge(gres.get_group(i), gmeta.get_group(j), how='inner', on='ID')
      if mx['max'] < len(m): mx = {'m_id': j, 'r_id': i, 'max': len(m)}
  ...
  r = {'hi': np.max(acc['acc']), 'lo': np.min(acc['acc']), 'wt_avg': np.average(acc['acc'], weights=acc['max']), 'avg': np.average(acc['acc']), 'nc': int(np.max(acc['r_id']))}
  return r
...
def coverage(res):
  ...
  for i in gmeta.groups:
    ...
    for j in gres.groups:
      m = pd.merge(gres.get_group(j), gmeta.get_group(i), how='inner', on='ID')
      if mx['max'] < len(m): mx = {'m_id': j, 'r_id': i, 'max': len(m)}
      mm = {'m_id': j, 'r_id': i, 'max': len(m), 'acc': len(m)/len(gmeta.get_group(i))}
    mx['cov'] = mx['max']/len(gmeta.get_group(i))
    cov = cov.append(mx, ignore_index=True)
  r = {'hi': np.max(cov['cov']), 'lo': np.min(cov['cov']), 'wt_avg': np.average(cov['cov'], weights=cov['max']), 'avg': np.average(cov['cov'])}
  return r
...
def analyze(input_file_name, files):
  meta = pd.read_table(input_file_name + '_meta')[['ID','Group']] # '  '_meta is meta file corresponding to input to clustering
